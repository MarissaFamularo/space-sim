...
| `spacesim.pelican.v1` | `{ v: 1, unlocked: bool, best: int }` — the 🔨 Rocket Forge duel prize state (2026-08-02) | contest.js `savePelican()` — main.js forge win handler + personal-best updates | contest.js `loadPelican()`; builder.js palette (📁 PELICAN button), copilot.js snapshot `forge` field; parsed by pure `parsePelicanSave` (node-tested, tests/contest_test.mjs) | Any junk → `{unlocked:false, best:0}` (duel simply not won yet), never a crash. Rule 2: new key, the five older keys untouched. |
