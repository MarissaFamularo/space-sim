...
| `spacesim.crew.v1` | `{ pick: ConnieName \| null, log: { [name]: missionsFlown } }` — pick null = Surprise-me/random; counts clamped 1..99999, max 64 names (2026-07-12) | crew.js `save()` via `Crew.setPick()` (Astronaut Complex / 🐍 Crew button) and `Crew.recordMission()` (main.js `assignCrew()` on every crewed launch/teleport) | crew.js `Crew.chooseForLaunch()/missions()/roster()`; copilot.js snapshot `flight.crew.missions` | `normalizeCrewData()` (pure, node-tested): garbage → `{pick:null, log:{}}`; a pick naming a deleted Connie falls back to random — a stale pick can never break a launch |
