...
| `spacesim.crew.v1` | `{ picks: [ConnieName,...], log: { [name]: missionsFlown } }` — picks is the ordered crew LINEUP (max 3, [0]=commander, [] = Surprise-me/random); counts clamped 1..99999, max 64 names (2026-07-12; the same-day interim `pick: name` shape is migrated at load) | crew.js `save()` via `Crew.togglePick()/clearPicks()` (Astronaut Complex / 🐍 Crew button) and `Crew.recordMission()` (main.js `assignCrew()` logs every member on every crewed launch/teleport) | crew.js `Crew.chooseCrewForLaunch(seats)/missions()/roster()/nextRecruitInfo()`; copilot.js snapshot `flight.crew.missions` + `flight.crewAll` | `normalizeCrewData()` (pure, node-tested): garbage → `{picks:[], log:{}}`; stale names in the lineup are skipped and seats fill randomly — a stale pick can never break a launch. crew.js also READS `spacesim.science.v1` (never writes) to gate connies.js RECRUITS milestones |
